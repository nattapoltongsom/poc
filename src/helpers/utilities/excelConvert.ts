...
import { ErrorField } from "../errors/bad-request-error";
...
export class ExcelConverter {
  ...
  private headerSelectDataObject(): ExcelDataObject {
    const headerNameSelect = this._headerSelect.map((e) =>
      e.name.toLocaleLowerCase()
    );
    const headerRequire = this._headerSelect.filter(
      (e) => e.isValueRequired === true
    );
    const headerNameRequire = headerRequire.map((e) =>
      e.name.toLocaleLowerCase()
    );
    const headerFormat = this._headerSelect.filter(
      (e) => e.format != undefined
    );
    ...
    const header = this._excelDataHeader
      ...
      .map((row) => {
        const isRequire = headerNameRequire.includes(
        ...
        const formats = headerFormat.find(
          (e) => e.name.toLocaleLowerCase() === row.name.toLocaleLowerCase()
        );
        if (formats) {
